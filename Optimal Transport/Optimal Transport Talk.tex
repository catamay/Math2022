...
\frame{\frametitle{Discussing Informal Exchange \& Applications of Duality}
\begin{block}<+->{Informal$\star$}
...
Message me at kirby\#4923 with specific questions.
\end{block}
\begin{block}<+->{Applications}
	Duality is important in the existence of transport maps (Brenier Polar Factorization).\newline
	The Monge-Kantorovich problem also can be shown to be a "continuous" linear programming problem. The proof of which is in "Partial Differential Equations and
	Monge-Kantorovich Mass Transfer" by Evans.
\end{block}
}
\section{Wasserstein}
